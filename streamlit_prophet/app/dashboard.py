...
# Info
with st.expander(
    "Effortlessly Forecast Time Series with Our Streamlit App", expanded=False
):
...
#add data into expander
with st.sidebar.expander(
    "Columns Configuration", expanded=False
):
    st.write(readme["app"]["columns"])
    st.write("")
st.write("")
# Column names
...
with st.sidebar.expander(
    "Filtering Options", expanded=False
):
...
#  eval
with st.sidebar.expander(
    "Evaluation Options", expanded=False
):
...
#  forcast
with st.sidebar.expander(
    "Forecast Options", expanded=False
):
